class MusicPlayer {
  ...
  int queueClickCount = 0;
  int queueLastClick = 0;
  ...
  void playNextMiniQueueSong() {
    if (miniPlayOn && queueSize > 0) {
      currentIndex = miniQueue[0];
      for (int i = 0; i < queueSize - 1; i++) {
        miniQueue[i] = miniQueue[i + 1];
      }
      miniQueue[queueSize - 1] = -1;
      queueSize--;
    } else {
      if (miniPlayOn && queueSize == 0) {
        miniPlayOn = false;
  ...
  color getButtonColor(int i) {
    ...
    if (i == 7) {
      if (shuffleOn) return red;
    }
    if (i == 8) {
      if (miniPlayOn) return red;
    }
  ...
  void buttonPressed(int i) {
    ...
    if (i == 2) { // play / pause / stop
      int now = millis();
      if (now - playLastClick > doubleClickDelay) {
        playClickCount = 0;
      }
      playClickCount++;
      playLastClick = now;
    ...
    if (i == 8) {
      int now = millis();
      if (now - queueLastClick > doubleClickDelay) {
        queueClickCount = 0;
      }
      queueClickCount++;
      queueLastClick = now;
      ...
      if (queueClickCount == 2) {
        if (!miniPlayOn) {
          savedSongIndex = currentIndex;
          miniPlayOn = true;
        } else {
          miniPlayOn = false;
        }
